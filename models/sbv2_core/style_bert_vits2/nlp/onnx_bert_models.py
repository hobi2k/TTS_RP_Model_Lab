...
def _resolve_onnx_model_path(pretrained_model_name_or_path: str, cache_dir: Optional[str], revision: str) -> Path:
    """
    Resolve ONNX BERT model path with robust fallback order:
    1) model_fp16.onnx
    2) model.onnx
    """
    candidates = ("model_fp16.onnx", "model.onnx")

    # Hugging Face repo id (aaaa/bbbb)
    if len(pretrained_model_name_or_path.split("/")) == 2:
        ...
        for filename in candidates:
            try:
                ...
                last_error = e
        assert False, (
            f"Failed to download ONNX BERT model from {pretrained_model_name_or_path}. "
            f"Tried files: {candidates}. Last error: {last_error}"
        )

    # Local directory path
    base = Path(pretrained_model_name_or_path).resolve()
    for filename in candidates:
        candidate = base / filename
        if candidate.exists():
            return candidate
    assert False, (
        f"ONNX BERT model not found under {base}. "
        f"Tried files: {candidates}"
    )
...
def load_model(
    ...
    # ONNX BERT モデルパスを解決 (model_fp16.onnx -> model.onnx の順でフォールバック)
    model_path = _resolve_onnx_model_path(
        pretrained_model_name_or_path=pretrained_model_name_or_path,
        cache_dir=cache_dir,
        revision=revision,
    )
    # 英語用 BERT のみ、spm.model もダウンロードする (Hugging Face repo 指定時)
    if len(pretrained_model_name_or_path.split("/")) == 2 and language == Languages.EN:
        hf_hub_download(
            repo_id=pretrained_model_name_or_path,
            filename="spm.model",
            cache_dir=cache_dir,
    ...
    logger.info(
        f"Loaded the {language.name} ONNX BERT model from {model_path} ({time.time() - start_time:.2f}s)"
    )
