...
- `/models/Style-BERT-VITS2`

현재 shim은 아래 파일들에 들어 있다.

- [style_gen.py](/models/Style-BERT-VITS2/style_gen.py)
- [slice.py](/models/Style-BERT-VITS2/slice.py)
...
반면 `pyannote.audio`와 `silero-vad`는 import 시점과 런타임에 여전히 아래 항목들을 기대한다.
...
1. `pyannote.audio`와 `silero-vad`가 기대하는 구 `torchaudio` API를 최소한으로 복원해 `style_gen.py`와 `slice.py`를 계속 실행 가능하게 만드는 것
2. PyTorch nightly의 강화된 기본 보안 정책 아래에서도 신뢰 가능한 `pyannote` 체크포인트를 로드할 수 있게 하는 것

## 2. 현재 적용된 shim

`style_gen.py`와 `slice.py`에서 상위 라이브러리를 import하기 전에 `torchaudio`를 먼저 import한 뒤, 필요한 심볼이 없으면 동적으로 채워 넣는다.
...
이를 피하기 위해 `style_gen.py`와 `slice.py`는 `torchaudio.load`를 `soundfile.read()` 기반 호환 함수로 덮어쓴다.
...
- `pyannote.audio`와 `silero-vad`가 `torchaudio.load(...)`를 호출해도 `torchcodec` 경로를 타지 않는다.
- Step 5 스타일 특징 추출에 필요한 WAV 로드는 계속 가능하다.
- Step 2 오디오 슬라이싱에서 `silero-vad`가 WAV를 읽을 때도 계속 동작한다.

### 2.7 `slice.py` 적용 이유

[slice.py](/models/Style-BERT-VITS2/slice.py)는 `torch.hub.load(...)`로 `litagin02/silero-vad`를 불러온다.

이 라이브러리는 현재 nightly `torchaudio`에서 제거된 레거시 top-level I/O API를 참조한다.

실제 발생한 오류:

- `module 'torchaudio' has no attribute 'list_audio_backends'`

즉, `slice.py`의 shim은 `silero-vad`가 내부적으로 기대하는 구 `torchaudio` API를 만족시키기 위한 것이다.
...
- `silero-vad` 로드 시 `torchaudio.list_audio_backends()` 호출 성공
- Step 2 오디오 슬라이싱 단계 진행
...
- `pyannote.audio` 또는 `silero-vad`가 이후 단계에서 추가로 제거된 API를 참조하는 경우
...
1. `pyannote.audio`와 `silero-vad`가 최신 `torchaudio` API를 공식 지원하는 버전으로 올라가기
2. 현재 nightly가 아닌, `pyannote.audio`/`silero-vad`와 API 호환되는 `torchaudio` 버전으로 내리기
3. `style_gen.py` 구현을 `pyannote.audio` 의존 없이 다른 스타일 임베딩 방식으로 교체하기
4. `slice.py`의 VAD 경로를 `silero-vad` 외 다른 방식으로 교체하기
5. `torchcodec`를 포함한 정식 오디오 스택과 `pyannote`/`silero-vad`가 nightly에서 완전히 호환되도록 버전 정렬하기
...
4. `slice.py`가 더 이상 `silero-vad`에 의존하지 않게 될 때
5. `Model.from_pretrained(...)`가 별도 `add_safe_globals(...)` 없이도 현재 체크포인트를 읽을 수 있을 때
...
- `style_gen.py` 실행 전에는 [style_gen.py](/models/Style-BERT-VITS2/style_gen.py)의 shim 코드가 제거되지 않았는지 확인한다.
- `slice.py` 실행 전에는 [slice.py](/models/Style-BERT-VITS2/slice.py)의 shim 코드가 제거되지 않았는지 확인한다.
...
- `silero-vad` 관련 새 `AttributeError`가 나오면, 동일하게 제거된 `torchaudio` 레거시 API인지 먼저 확인한다.
