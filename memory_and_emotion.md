...
# Memory + Emotion DB 조회 가이드

대상 DB:
- `outputs/memory/memory.sqlite3`

## 1) 일반 테이블 (CLI로 바로 조회 가능)
...
## 2) 벡터 테이블 (sqlite-vec 필요)

`memory_slot_vec`는 `vec0` 확장을 로드해야 접근할 수 있다. 
CLI에서 바로 조회하려면 확장 경로를 명시해야 한다.
...
- vec가 안 만들어지면 `sqlite_vec.load()`가 실패한 것일 가능성이 크다.
- 앱 로그에서 다음이 찍히면 정상:
```
