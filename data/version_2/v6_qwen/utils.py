...
사용 예시:
uv run data/v5_qwen_multiturn_gen.py \
  ...
  --fsm_path data/v5_qwen_fsm.yaml \
...
# 주인공 이름 오기(타 샘플 이름 침투) 보정을 위한 기본 후보군.
# 실제 주인공 이름이 이 목록에 있으면 그대로 유지하고, 다른 후보가 섞이면 현재 이름으로 통일한다.
...
def assistant_sampling_params(
    ...
    *,
    ...
    """
    # 동일 실패가 누적될수록 샘플링 폭을 단조 하향해 형식/역할 안정성을 우선한다.
...
def user_sampling_params(
    ...
    *,
    ...
    user 생성이 말투 불일치 또는 형식 오류로 반복될 때,
    재시도 횟수에 따라 샘플링 폭을 점진적으로 조정해
    규칙 순응도를 높인다.

    Args:
        retry_user: 현재 user 재시도 누적 횟수.
        base_temperature: 기본 temperature.
        base_top_p: 기본 top_p.

    Returns:
        Dict[str, float]: user 생성에 사용할 샘플링 파라미터.
    """
    # user는 실패 누적 시 샘플링을 단조 하향해 장문/역할 이탈을 줄인다.
    if retry_user >= 10:
        return {"temperature": 0.36, "top_p": 0.68}
    if retry_user >= 6:
        return {"temperature": 0.44, "top_p": 0.74}
...
def normalize_protagonist_refs(text: str, protagonist: str) -> str:
    ...
    `주인공` 같은 일반 지칭을 실제 주인공 이름으로 치환한다.
    """
    if not text:
        return ""
    t = text
...
def parse_protagonist_name(system_lore: str) -> str:
    ...
    # 괄호 없는 서술형 문장도 주인공 이름으로 파싱한다.
    m1b = re.search(r"당신은\s*이제\s*([^\s\.,:;\"'“”]{1,20})\s*입니다", t)
    if m1b:
        return m1b.group(1).strip()

    # 이름 라인 fallback은 반드시 "3. 주인공 정의" 블록을 우선한다.
    # 전역에서 첫 "이름:"을 잡으면 플레이어 이름이 주인공으로 오염될 수 있다.
    m_block = re.search(r"3\.\s*주인공\s*정의([\s\S]*?)(?:\n\s*4\.\s*|$)", t)
    if m_block:
    ...
    # 블록 파싱 실패 시에만 전역 fallback을 사용한다.
    m2g = re.search(r"(?:[-•]\s*)?이름\s*[:：]\s*([^\n]{1,24})", t)
    if m2g:
        name = m2g.group(1).strip()
        name = re.sub(r"[「」『』《》\"']", "", name)
        name = re.split(r"[\(\[]", name)[0].strip()
        if name:
            return name[:20]
...
def parse_player_name(system_lore: str) -> str:
    ...
    if m3:
        ...
        return name[:20]
    # 역할/최근 상호작용 문장에서 "플레이어(이름)" 표기를 fallback으로 추출한다.
    m4 = re.search(r"플레이어\s*[\(\[]\s*([^\)\]\n]{1,20})\s*[\)\]]", t)
    if m4:
        name = re.sub(r"[「」『』《》\"']", "", m4.group(1).strip())
        name = re.split(r"[\(\[]", name)[0].strip()
        if name:
            return name[:20]
