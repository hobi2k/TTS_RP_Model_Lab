...
Run:
  uv run data/v7_qwen_charcard_gen.py \
...
class PhraseBiasLogitsProcessor(LogitsProcessor):
    """`PhraseBiasLogitsProcessor`의 데이터 구조와 동작 계약을 정의하는 클래스다."""
    ...
    def _contains_any(self, seq):
        """내부 헬퍼로 `_contains_any` 계산 절차를 수행한다."""
...
class StopSequencesCriteria(StoppingCriteria):
    """`StopSequencesCriteria`의 데이터 구조와 동작 계약을 정의하는 클래스다."""
...
def generate_scenario_book(model, tokenizer, max_tokens: int, chosen_speech_style: Optional[str] = None) -> str:
...
@torch.inference_mode()
def _chat_generate_local(model, tokenizer, chat: list[dict], max_tokens: int, temperature: float = 0.2, top_p: float = 0.9) -> str:
    """Qwen 로컬 모델로 단일 채팅 프롬프트를 생성해 텍스트를 반환한다."""
    ...
    with open(args.out_path, "a", encoding="utf-8") as f:
        ...
        while accepted < args.samples:
            ...
            if has_speech_style_mismatch(text):
                ...
                if recent_line:
                    rewritten_line = rewrite_recent_turn_line(model, tokenizer, recent_line, chosen_speech_style).strip()
