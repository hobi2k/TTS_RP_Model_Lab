...
def llm_classify_dialogue_style_openai(
    ...
    """LLM 호출로 `llm_classify_dialogue_style_openai` 판정 결과를 계산한다."""
    prompt_msgs = [
        {
            "role": "system",
            "content": (
...
def llm_speech_style_mismatch_openai(
    ...
    """LLM 호출로 `llm_speech_style_mismatch_openai` 판정 결과를 계산한다."""
...
def llm_narration_plain_mismatch_openai(
    client: OpenAI,
    model_name: str,
    narration_text: str,
) -> bool:
    ...
    if not normalize_space(narration_text):
        return False
    style = llm_classify_dialogue_style_openai(client, model_name, narration_text)
...
def llm_dialogue_quality_fail_openai(
    ...
    *,
    ...
    """LLM 호출로 `llm_dialogue_quality_fail_openai` 판정 결과를 계산한다."""
    prompt_msgs = [
        {
            "role": "system",
            "content": (
...
def llm_role_conflict_fail_openai(
    client: OpenAI,
    model_name: str,
    *,
    system_lore: str,
    ...
    for ln in (raw or "").splitlines():
        tok = re.sub(r"[^A-Z]", "", ln.strip().upper())
        if not tok:
            continue
...
def llm_user_role_conflict_fail_openai(
    client: OpenAI,
    model_name: str,
    *,
    system_lore: str,
...
def user_invalid_reason(text: str, *, player_name: str = "", protagonist: str = "") -> Optional[str]:
    ...
    # MOD: 플레이어 이름이 누락될 수 있어도 형식 위반은 아니다.
