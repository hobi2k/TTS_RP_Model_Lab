...
# GPT 경로도 Qwen 프롬프트를 공통 사용해 프롬프트 불일치를 제거한다.
from prompts import (
...
from validators import (
    ...
    llm_dialogue_quality_fail_openai,
    llm_role_conflict_fail_openai,
    llm_stall_detect_openai,
    llm_user_role_conflict_fail_openai,
...
# Sexual/aftermath pacing controls (v7 단발 전개용 가속)
MIN_SEXUAL_TURNS = 2
...
def _is_repeat_intent_no_progress(
    *,
    curr_dialogue: str,
    prev_dialogue: str,
    curr_intent: str,
    ...
    """동일 의도 반복 + 저진전(고유 서명/의미 유사) 여부를 판정한다."""
    if not curr_dialogue or not prev_dialogue:
        return False
    if not curr_intent or not prev_intent:
        return False
    if curr_intent != prev_intent:
        return False
    if curr_signature and prev_signature and curr_signature == prev_signature:
        return True
    return is_dialogue_echo(curr_dialogue, prev_dialogue, threshold=0.7)


def _must_force_progress(state: VNState) -> bool:
    """정체 감지 시 강한 전개 제약을 적용할지 반환한다."""
    if int(state.get("turns_target", 0) or 0) <= 4:
        return True
...
def choose_relation_intent(state: VNState, fsm_state: str, action_state: str) -> str:
    ...
    랜덤 의도만 사용하면 대화 흐름이 흔들리므로 국면/신호 기반 기본값을 사용한다.
...
def build_graph(
    ...
    def init_node(state: VNState) -> VNState:
        ...
        genre_now = parse_genre(state["system_lore"])
        forced_rel_state = {
            "연애물": "FLIRT",
            "육성물": "TRAINING",
            "비극": "TRAGEDY",
        ...
        if forced_rel_state:
            fsm_rel.state = forced_rel_state
        ...
        if genre_now == "성인물":
            if state["relation_status"] not in ("친밀", "사랑"):
                state["relation_status"] = "친밀"
            state["sexual_condition_rel"] = state["relation_status"]
            state["allow_sexual"] = True
        ...
        log_step("INIT", f"forced_fsm_state={fsm_rel.get_state()} genre={genre_now}")
    ...
    def gen_user_node(state: VNState) -> VNState:
        ...
        task += (
            "\n[추가 강제]\n"
            "- 이번 턴에는 직전 발화에 없던 구체 정보 1개를 반드시 포함한다.\n"
            "- 이번 턴에는 새로운 행동 1개를 반드시 포함한다.\n"
            "- 직전 발화 핵심 구절을 바꿔 말하며 반복하지 않는다.\n"
        )
        ...
        if err:
            ...
            return state
        # 1차 생성에서는 과검출을 줄이기 위해 역할 LLM 판정을 건너뛴다.
        if int(state.get("retry_user", 0) or 0) >= 1:
            if llm_user_role_conflict_fail_openai(
                client=client,
                model_name=model_name,
        ...
        curr_user_intent = _intent_key(curr_user_dia)
        curr_user_sig = _content_signature(curr_user_dia)
        if _is_repeat_intent_no_progress(
            curr_dialogue=curr_user_dia,
            prev_dialogue=state.get("last_user_dialogue", ""),
            curr_intent=curr_user_intent,
            prev_intent=str(state.get("last_user_intent", "") or ""),
            curr_signature=curr_user_sig,
            prev_signature=str(state.get("last_user_signature", "") or ""),
        ):
            state["retry_user"] += 1
            state["error"] = "user_repeat_turn"
            log_step("GEN_USER", "repeat=user_intent")
            log_retry(
                node="GEN_USER",
                retry=state["retry_user"],
                max_retry=30,
        ...
        if is_redundant_turn_dialogue(curr_user_dia, state.get("last_user_dialogue", "")):
            ...
            return state
        if is_dialogue_echo(curr_user_dia, state.get("last_asst_dialogue", ""), threshold=0.8):
            state["retry_user"] += 1
            state["error"] = "user_echo_assistant"
            log_step("GEN_USER", "invalid=user_echo_assistant")
            log_retry(
                node="GEN_USER",
                retry=state["retry_user"],
                max_retry=30,
    ...
    def gen_user_sexual_node(state: VNState) -> VNState:
        ...
        if _is_repeat_intent_no_progress(
            ...
            state["retry_user"] += 1
            state["error"] = "user_repeat_turn"
            log_step("GEN_USER_SEXUAL", "repeat=user_intent")
            log_retry(
                node="GEN_USER_SEXUAL",
                retry=state["retry_user"],
                max_retry=30,
        ...
        if is_dialogue_echo(curr_user_dia, state.get("last_asst_dialogue", ""), threshold=0.8):
            state["retry_user"] += 1
            state["error"] = "user_echo_assistant"
            log_step("GEN_USER_SEXUAL", "invalid=user_echo_assistant")
            log_retry(
                node="GEN_USER_SEXUAL",
                retry=state["retry_user"],
                max_retry=30,
    ...
    def gen_assistant_node(state: VNState) -> VNState:
        ...
        curr_asst_intent = _intent_key(dia)
        curr_asst_sig = _content_signature(dia)
        if _is_repeat_intent_no_progress(
            curr_dialogue=dia,
            prev_dialogue=state.get("last_asst_dialogue", ""),
            curr_intent=curr_asst_intent,
            prev_intent=str(state.get("last_asst_intent", "") or ""),
            curr_signature=curr_asst_sig,
            prev_signature=str(state.get("last_asst_signature", "") or ""),
        ):
            state["retry_assistant"] += 1
            state["error"] = "asst_repeat_turn"
            state["rewrite_mode"] = "normal"
            log_step("GEN_ASSISTANT", "repeat=asst_intent")
            log_retry(
                node="GEN_ASSISTANT",
                retry=state["retry_assistant"],
                max_retry=30,
            ...
            return state
        user_dia = extract_single_quote(state.get("user_text", "")) or ""
        if is_dialogue_echo(dia, user_dia, threshold=0.8):
            state["retry_assistant"] += 1
            state["error"] = "asst_echo_user"
            log_step("GEN_ASSISTANT", "invalid=asst_echo_user")
            log_retry(
    ...
    def eval_dialogue_quality_node(state: VNState) -> VNState:
        ...
        assistant 출력의 역할 충돌/품질 저하를 검사하되,
        v6 호환 정책에 따라 실패는 hard-fail이 아닌 경고로만 기록한다.
        ...
        role_failed = llm_role_conflict_fail_openai(
            client=client,
            model_name=model_name,
            system_lore=state.get("system_lore", ""),
            protagonist=state.get("protagonist", ""),
    ...
    def gen_crisis_node(state: VNState) -> VNState:
        ...
        if err:
            ...
            return state
        curr_asst_dia = _extract_dialogue_line(text)
        curr_asst_intent = _intent_key(curr_asst_dia)
        curr_asst_sig = _content_signature(curr_asst_dia)
        if _is_repeat_intent_no_progress(
            curr_dialogue=curr_asst_dia,
            prev_dialogue=state.get("last_asst_dialogue", ""),
            curr_intent=curr_asst_intent,
            prev_intent=str(state.get("last_asst_intent", "") or ""),
            curr_signature=curr_asst_sig,
            ...
            state["error"] = "crisis_repeat_turn"
            log_step("GEN_CRISIS", "repeat=asst_intent")
            log_retry(
                node="GEN_CRISIS",
                retry=state["retry_crisis"],
                max_retry=30,
    ...
    def gen_sexual_node(state: VNState) -> VNState:
        ...
        if _is_repeat_intent_no_progress(
            ...
            state["error"] = "asst_repeat_turn"
            state["rewrite_mode"] = "sexual"
            log_step("GEN_SEXUAL", "repeat=asst_intent")
            log_retry(
                node="GEN_SEXUAL",
                retry=state["retry_sexual"],
                max_retry=30,
    ...
    def eval_json_node(state: VNState) -> VNState:
        ...
        genre_weights = {
            ...
            "성인물": {
                "mental_instability": (0.15, 0.55, 0.30),
                "threat": (0.10, 0.60, 0.30),
                "pressure": (0.20, 0.55, 0.25),
                "probe": (0.20, 0.55, 0.25),
                "resolve": (0.35, 0.45, 0.20),
                "event": (0.55, 0.35, 0.10),
            },
            "비극": {
    ...
    def fsm_step_node(state: VNState) -> VNState:
        ...
        if prev_action_state in SEXUAL_STATES or base_action_state in SEXUAL_STATES:
            ...
            if base_action_state in CRISIS_STATES or base_action_state in AFTERMATH_CRISIS_STATES:
                base_action_state = "EVENT"
            state["action_state"] = base_action_state
            sexual_turns = 0
            aftermath_turns = 0
            sexual_lock = 0
            crisis_turns = 0
    ...
    def next_node(state: VNState) -> VNState:
        ...
        llm_stalled = llm_stall_detect_openai(
            client=client,
            model_name=model_name,
            system_lore=state.get("system_lore", ""),
            action_state=state.get("action_state", ""),
    ...
    def after_eval_dialogue_quality(state: VNState) -> str:
        """품질/역할 LLM 실패를 하드 실패로 처리해 재작성/재생성 분기한다."""
        err = str(state.get("error", "") or "")
        if not err:
            return "EVAL_INTERNAL"
        if err in ("asst_role_conflict_llm", "asst_dialogue_quality_llm"):
            action_state = state.get("action_state", "")
            if state.get("retry_quality", 0) < REWRITE_MAX_ATTEMPTS:
                if action_state in CRISIS_STATES or action_state in AFTERMATH_CRISIS_STATES:
                    state["rewrite_mode"] = "crisis"
                elif action_state in SEXUAL_STATES or action_state in AFTERMATH_SEX_STATES:
                    state["rewrite_mode"] = "sexual"
                else:
                    state["rewrite_mode"] = "normal"
                return "GEN_ASSISTANT_REWRITE"
            if action_state in CRISIS_STATES or action_state in AFTERMATH_CRISIS_STATES:
                return "GEN_CRISIS" if state.get("retry_crisis", 0) < 12 else "DONE"
            if action_state in SEXUAL_STATES or action_state in AFTERMATH_SEX_STATES:
                return "GEN_SEXUAL" if state.get("retry_sexual", 0) < 30 else "DONE"
            return "GEN_ASSISTANT" if state.get("retry_assistant", 0) < 30 else "DONE"
        return "EVAL_INTERNAL"
