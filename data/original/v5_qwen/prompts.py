...
- 매 턴 새로운 행동/결정/요구를 포함해 장면을 1단계 전진시킨다.
{ban_question_rule}
{ban_silence_rule}
{force_progress_rule}
...
- 관계 변화 의도가 '관계 상승'일 때:
  - 신뢰/공감/지지/접근 중 1개 필수
- 매 턴 세계관 요소(배경 장소, 역할, 제약, 사건 등) 1개 이상을 구체적으로 언급한다.
...
""")
    ...
    return tmpl.format(
        ...
        force_progress_rule="- 이번 턴은 반드시 구체 행동/결정/요구로 장면을 전진시킨다." if force_progress else "",
    ).strip()
...
- 서술은 주인공을 이름으로 지칭한다.
- 서술은 평어체로 작성한다.
- 두 번째 줄은 주인공의 대사이며 큰따옴표 1쌍만 허용한다.
