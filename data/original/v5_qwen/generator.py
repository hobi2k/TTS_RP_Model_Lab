...
def generate_text(
    ...
    """
    중요:
    - tokenizer가 token_type_ids를 만들 수 있다.
    - Tri 계열/일부 CausalLM은 token_type_ids를 generate kwargs로 받지 않아 폭발한다.
    - 따라서 inputs에서 token_type_ids는 항상 제거한다.
    """
...
def build_graph(
    ...
    Args:
        model: 본문 생성에 사용하는 Causal LM 인스턴스.
        tokenizer: 모델 입력/출력 토크나이저.
    ...
    def gen_assistant_node(state: VNState) -> VNState:
        ...
        if llm_speech_style_mismatch_local(
            model=model,
            tokenizer=tokenizer,
