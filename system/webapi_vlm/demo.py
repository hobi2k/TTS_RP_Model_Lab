...
def _render_dialog_inner(record: dict[str, Any] | str | None) -> str:
    """대화 패널 내부 HTML을 구성한다."""
    if not record:
        return '<div class="speaker-tag">SAYA VLM</div><div class="vn-line">...</div>'
    ...
    parts = ['<div class="speaker-tag">SAYA VLM</div>']
    if narr:
...
def _render_history(history: list[dict[str, Any]]) -> str:
    ...
    for i, h in enumerate(history[-20:], start=max(1, len(history) - 19)):
        lines.append(f"{i:02d}. U: {h.get('user', '')}\n    A: {h.get('dialogue_ko', '')}")
    return "\n".join(lines)


def build_demo(base_url: str) -> gr.Blocks:
    """VLM 전용 Gradio Blocks 데모를 생성한다."""
    asset_root = Path(__file__).resolve().parents[1] / "webapi" / "assets"
    asset_neutral = asset_root / "neutral_full.png"
    asset_sad = asset_root / "crying_full.png"
    asset_happy = asset_root / "smile_full.png"
    asset_angry = asset_root / "annoyed_full.png"
    dev_face_path = asset_root / "face_neutral.png"
    bg_path = asset_root / "background.jpg"
    for path in [asset_neutral, asset_sad, asset_happy, asset_angry, dev_face_path, bg_path]:
        if not path.exists():
    ...
    def mainloop_turn(user_text: str, speaker_name: str, image_path: str | None, history: list[dict[str, Any]]):
        payload = {
            "text_ko": user_text,
            "speaker_name": (speaker_name or "saya"),
        ...
        try:
            ...
            yield _error_frame(f"main-loop failed ({exc.response.status_code}): {detail or str(exc)}", history or [])
            return
        except Exception as exc:
            yield _error_frame(f"main-loop exception: {exc}", history or [])
            return
        ...
        rec_partial = {**rec_full, "dialogue_ko": "", "dialogue_ja": ""}
        yield (
        ...
        for i in range(1, len(full_dialogue_ko) + 1):
    ...
    def chat_api(user_text: str, image_path: str | None) -> str:
        with httpx.Client(timeout=180.0) as client:
            res = client.post(f"{base_url}/api/chat", json={"text": user_text, "image_path": image_path})
            res.raise_for_status()
    ...
    with gr.Blocks(title="SAYA VLM MainLoop Demo") as demo:
        history_state = gr.State([])

        gr.HTML(
            f"<style>{CSS}</style>"
            '<div class="vn-root vn-head">'
            f'<h1 class="vn-title">SAYA VLM / MainLoop UI v{UI_VERSION}</h1>'
            f'<p class="vn-sub">REST backend: {html.escape(base_url)} | pipeline: input -> VLM -> parse -> translate -> emotion -> TTS</p>'
            "</div>"
        )
            ...
            with gr.Group(elem_id="vn-input-wrap"):
                with gr.Row(elem_id="vn-input-row"):
                    user_input = gr.Textbox(
                        show_label=False,
                        lines=1,
                        placeholder='VLM 대화 입력: 예) "사야, 오늘 기분 어때?"',
                        container=False,
        ...
        with gr.Accordion("Dev/Test Tools", open=False):
            ...
            with gr.Tab("Chat only"):
                ...
                c_image = gr.Image(label="Image", type="filepath", sources=["upload"], height=240)
                c_out = gr.Textbox(label="RP Output")
                c_btn = gr.Button("POST /api/chat")
                c_btn.click(chat_api, inputs=[c_in, c_image], outputs=c_out)
        ...
        run_btn.click(
            mainloop_turn,
            inputs=[user_input, speaker_name, image_input, history_state],
            outputs=[stage_html, audio_hidden, transcript, dbg_rp, dbg_narr, dbg_dko, dbg_dja, dbg_wav, history_state, user_input, image_input],
